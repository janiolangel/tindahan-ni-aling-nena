...
<head>
    <style>
        * {
        ...
        .sidebar .btn, .add-product, .add-invoice,  .menu, .delete-product{
            padding: 14px 20px;
            border-radius: 12px;
            background: rgba(255, 255, 255, 0.95);
            color: #667eea;
...
<body>
    <div class="container">
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])."?stock_id=".$_GET['stock_id'];?>" class="content-area">
            <div>
                <label class="logo-text">Delete <?php echo htmlspecialchars($name)?>?</label>
                <button type="submit" name="delete_product" class="btn delete-product">Delete</button>
            </div>
        </form>
